...
```
### 第三天  
第三天我们主要就是进行了实践，模拟了小车运行时发动机的状态及指示灯的亮灭，以及通过数码管的实验加强了对锁存器CD4511，数码管工作原理的了解
...
```
### 第四天
第四天老师主要是进行了答疑方面的工作，不过也给我们布置了关于锁存器灵活应用的作业，锁存器低电平写入数据，高电平保持。这样就可以使得CD4511输入端数据一样
而数码管显示数据却不一样。
![avatar](/图片/数码管图片.png)  
...
void loop()
{
  ...
  这个东西。arduino,frizting,processing,三个工具，github的使用，在线仿真的过程，代码编写的思路，还有最最重要的**动手实践**，这不仅对这门
